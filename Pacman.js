...
class Ghost {
  ...
  draw() {
    if (Pac.CanKill == false) ctx.fillStyle = this.color;
    ...
    if (this.SpeedX == -SpeedG) {
      ...
      for (let i = 0; i < MazeKeys.length; i++) {
        if (MazeKeys[i].y == this.y + 25) {
          let MazeKeysX = MazeKeys[i].x;
          if (
            MazeKeysX - (this.x + 25) > 0 &&
            MazeKeysX - (this.x + 25) < Dis
          ) {
      ...
      for (let i = 0; i < MazeKeys.length; i++) {
        if (MazeKeys[i].x == this.x + 25) {
          ...
          if (
            MazeKeysY - (this.y + 25) > 0 &&
            MazeKeysY - (this.y + 25) < Dis
          ) {
  ...
  Turn(DidGhostHaveToTurn) {
    ...
    if (this.SpeedX != 0) Commands = ["Up", "Down"];
    else if (this.SpeedY != 0) Commands = ["Left", "Right"];
    ...
    for (let i = 0; i < Commands.length; i++) {
      ...
      if (Command == "Up") {
        ...
        if (this.CanMoveToThere(true) == false) {
          ...
          if (DidGhostHaveToTurn) return true;
          return;
        }
    ...
    if (DidGhostHaveToTurn) return false;
  }
  ...
  draw() {
    if(this.SpeedX <= 0 && this.SpeedY == 0){
      ...
      ctx.arc(
        ...
        this.y,
        Radius,
        4 - (LestTop/20), //when up then it goes down and the oppsite
        2.50 + (LestBottom/35) //when up then it goes up and the oppsite
      ...
      //eyes
      ctx.beginPath();
      ctx.arc(this.x + 3, this.y - 10, 3, 0, 2 * Math.PI);
      ctx.fill();
      ctx.stroke();
      ctx.closePath();
    }
      ...
      ctx.arc(
        ...
        this.y,
        Radius,
        0.5 - (LestTop/30), //when up then it goes down and the oppsite
        -0.5 + (LestBottom/30)  //when up then it goes up and the oppsite
        //(LestBottom / 500) * Math.PI,
        //(LestTop / 100) * Math.PI
      );
      ...
      ctx.arc(
        ...
        this.y,
        Radius,
        -0.7 - (LestTop/20), //when up then it goes down and the oppsite
        4 + (LestBottom/20) //when up then it goes up and the oppsite
        //(LestBottom / 500) * Math.PI,
        //(LestTop / 100) * Math.PI
      );
      ...
      //eyes
      ctx.beginPath();
      ctx.arc(this.x + 10, this.y, 3, 0, 2 * Math.PI);
      ctx.fill();
      ctx.stroke();
      ctx.closePath();
    }
      ...
      ctx.arc(
        ...
        this.y,
        Radius,
        2.2 - (LestTop/20), //when up then it goes down and the oppsite
        1 + (LestBottom/20), //when up then it goes up and the oppsite
        //(LestBottom / 500) * Math.PI,
        //(LestTop / 100) * Math.PI
      );
...
let BoxGhostMaze = [
  { x: 750, y: 300, point: false },
  { x: 800, y: 300, point: false },
  { x: 850, y: 300, point: false },
  { x: 750, y: 350, point: false },
  { x: 800, y: 350, point: false },
  { x: 850, y: 350, point: false }
];

let SpotsThatCouldBeConnectedToMaze = [
  { x: 750, y: 250, SpotBox: { x: 750, y: 300, SpeedX: 0, SpeedY: -SpeedG } },
  { x: 700, y: 300, SpotBox: { x: 750, y: 300, SpeedX: -SpeedG, SpeedY: 0 } },
  { x: 800, y: 250, SpotBox: { x: 800, y: 300, SpeedX: 0, SpeedY: -SpeedG } },
  { x: 850, y: 250, SpotBox: { x: 850, y: 300, SpeedX: 0, SpeedY: -SpeedG } },
  { x: 900, y: 300, SpotBox: { x: 850, y: 300, SpeedX: SpeedG, SpeedY: 0 } },
  { x: 700, y: 350, SpotBox: { x: 750, y: 350, SpeedX: -SpeedG, SpeedY: 0 } },
  { x: 750, y: 400, SpotBox: { x: 750, y: 350, SpeedX: 0, SpeedY: SpeedG } },
  { x: 800, y: 400, SpotBox: { x: 800, y: 350, SpeedX: 0, SpeedY: SpeedG } },
  { x: 850, y: 400, SpotBox: { x: 850, y: 350, SpeedX: 0, SpeedY: SpeedG } },
  { x: 900, y: 350, SpotBox: { x: 850, y: 350, SpeedX: SpeedG, SpeedY: 0 } }
];
...
function setup() {
  ...
  document.onkeydown = (e) => Pac.CheckKey(e);
  ...
  for (let i = 0; i < NumberOfGhosts; i++) {
    Ghosts.push(
      new Ghost(
        TheConnectedSpot.x,
        TheConnectedSpot.y,
        TheConnectedSpot.SpeedX,
        TheConnectedSpot.SpeedY,
  ...
  for (let i = 0; i < Maze.length; i++) {
    ...
    if (check) MazeKeys.push({ x: Maze[i].x + 25, y: Maze[i].y + 25 });
  }
...
function CreateMaze(
  ...
  for (let i = 0; i < NumberOfStartingPoints; i++) {
    ...
    let XStartingP =
      Math.floor(Math.floor(Math.random() * (width - 50)) / MazeSize) * 50;
    let YStartingP =
      Math.floor(Math.floor(Math.random() * (height - 50)) / MazeSize) * 50;
    ...
    while (
      !LocStartingOkay(StartingPoints, XStartingP, YStartingP, MazeSize) ||
      CheckDouble(XStartingP, YStartingP, "StartingPoint")
    ) {
      XStartingP =
        Math.floor(Math.floor(Math.random() * (width - 50)) / MazeSize) * 50;
      YStartingP =
    ...
    for (let j = 0; j < NumberOfWalkers; j++) {
      ...
      let Steps =
        Math.floor(
          Math.random() * (MaximumNumberOfSteps - MinimumNumberOfSteps + 1)
        ) + MinimumNumberOfSteps;
      ...
      if (!NextStepOkay(x, y, CurrDir)) {
        ...
        switch (CurrDir) {
          case "Up":
            if (Math.random() > 0.5 && NextStepOkay(x, y, "Right"))
              CurrDir = "Right";
            else if (NextStepOkay(x, y, "Left")) CurrDir = "Left";
            else if (NextStepOkay(x, y, "Right")) CurrDir = "Right";
          ...
          case "Right":
            if (Math.random() > 0.5 && NextStepOkay(x, y, "Up")) CurrDir = "Up";
            else if (NextStepOkay(x, y, "Down")) CurrDir = "Down";
            else if (NextStepOkay(x, y, "Up")) CurrDir = "Up";
      ...
      for (let k = 0; k < Maze.length; k++) {
        ...
        switch (CurrDir) {
          case "Up":
            if (OverRide(x, y - MazeSize, Wx, Wy, MazeSize)) {
              Index = i;
              Skip = true;
            }
          ...
          case "Right":
            if (OverRide(x + MazeSize, y, Wx, Wy, MazeSize)) {
              Index = i;
              Skip = true;
            }
      ...
      if (Index == -1) {
        ...
        switch (CurrDir) {
          case "Up":
            ...
            Draw(
              x,
              y + (ColiideY + MazeSize - y),
              MazeSize,
              MazeSize - (ColiideY + MazeSize - y)
            );
...
function SimulateWalkers(Walkers) {
  while (Walkers.length != 0) {
    ...
    for (let i = Walkers.length - 1; i > -1; i--) {
      ...
      if (CurrDir == "Up") Draw(x, y, MazeSize, MazeSize, "lightblue");
      else if (CurrDir == "Down") Draw(x, y, MazeSize, MazeSize, "yellow");
      else if (CurrDir == "Right") Draw(x, y, MazeSize, MazeSize, "white");
      else if (CurrDir == "Left") Draw(x, y, MazeSize, MazeSize, "green");
...
function CheckDouble(x, y, CurrDir) {
  for (let i = 0; i < Maze.length; i++) {
    ...
    switch (CurrDir) {
      case "Up":
        if ((x + MazeSize == Wx && y == Wy) || (x - MazeSize == Wx && y == Wy))
          return true;
        break;
      ...
      case "Right":
        if ((x == Wx && y + MazeSize == Wy) || (x == Wx && y - MazeSize == Wy))
          return true;
        break;
...
function NextStepOkay(x, y, dir) {
  switch (dir) {
    case "Up":
      if (y - MazeSize >= 0) return true;
      else return false;

      break;

    case "Down":
      if (y + MazeSize * 2 <= height) return true;
      else return false;

      break;

    case "Right":
      if (x + MazeSize * 2 <= width) return true;
      else return false;

      break;

    case "Left":
      if (x - MazeSize >= 0) return true;
      else return false;
...
function LocStartingOkay(Points, x, y) {
  for (let i = 0; i < Points.length; i++)
    if (OverRide(x, y, Points[i].x, Points[i].y)) return false;
...
function OverRide(x1, y1, x2, y2) {
  if (
    (x1 < x2 && x2 < x1 + MazeSize && y1 < y2 && y2 < y1 + MazeSize) ||
...
function DrawPoints(Firstime) {
  ...
  if (Firstime) {
    ...
    for (let i = 0; i < Points.length; i++) {
      let Pos = Points[i];
      if (Pos.Kill == false) DrawPoint(Pos.x, Pos.y, PointR, "red");
      else DrawPoint(Pos.x, Pos.y, PointR + 5, "blue");
    }
  }
}
...
function saveScore(){
  ...
  for(let i in users){
    if(users[i].name === currentUser){
      if(users[i].score < Score) users[i].score = Score;
      break;
    }
  }
